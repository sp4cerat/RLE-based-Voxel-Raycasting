...
uniform vec3 res_x_y_ray;

void main(void)
{
   float RESX  = res_x_y_ray.x;//1024.0;
   float RESY  = res_x_y_ray.y;//768.0;
   float MAXRAY= res_x_y_ray.z;//2048.0;
...
//	   +      (seg_lt        ) * ( 1.0-o2 );
   ...
   float col16r=float(((x2>>10)&31))/31.0;//+((x2>>5)<<(3+8))+((x2>>5)<<(3+8))
	...
	c=vec4(0.3,0.3,0.3,1.0)*(1.0-scy)+vec4(0.8,0.8,0.8,1.0)*(scy);
